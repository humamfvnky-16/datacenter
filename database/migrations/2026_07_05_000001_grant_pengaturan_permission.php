...
/**
 * Data migration:
 *  - Tambah permission baru: pengaturan/*
 *  - Sync ke role 'admin' dan 'super-admin' yang sudah ada di DB (TIDAK ke
 *    'operator', mengikuti konvensi 'sekolah/*' — pengaturan aplikasi/branding
 *    adalah wewenang admin, bukan operator data).
 *
 * Tanpa migration ini, admin/super-admin yang akunnya terhubung ke role custom
 * (role_id terisi) akan mendapat 403 saat membuka menu Pengaturan > Halaman
 * Login, karena HasRbac::getPermissions() memakai daftar permission eksplisit
 * dari role tersebut (bukan wildcard '*') begitu role_id ter-set — lihat
 * app/Concerns/HasRbac.php. Fitur baru selalu perlu didaftarkan seperti ini,
 * mengikuti pola migration 2026_07_02_000001_grant_periodikal_permission.php.
 *
...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $permission = DB::table('permissions')->where('permission', 'pengaturan/*')->first();
        if (! $permission) {
            $permId = DB::table('permissions')->insertGetId([
                'permission' => 'pengaturan/*',
                'label' => 'Kelola Pengaturan (Halaman Login, dsb)',
                'group' => 'datacenter',
                'created_at' => now(),
                'updated_at' => now(),
            ]);
    ...
    public function down(): void
    {
        ...
        $permId = DB::table('permissions')->where('permission', 'pengaturan/*')->value('id');
